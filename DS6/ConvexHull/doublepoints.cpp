...
bool pointsInSameLine(ExtendedDouble<Point>& points, ExtendedDouble<Point>& endpoints, Point& pole) {
	if (points.size() <= 2) {
		endpoints = points;
		return true;
	}
	...
	ExtendedDouble<Point>::iterator b = points.begin();
	...
	if (result) {//in the same line, calculate the endpoints
		...
		ExtendedDouble<Point>::iterator b2 = points.begin();
